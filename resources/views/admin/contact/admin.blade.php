                        ...
                        <form action="{{ route('department.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="">
                                <div class="form-group">
                                    ...
                                    <input name="department" type="hidden" value="admin" class="form-control"
                                        placeholder="">
        ...
        @else
            ...
            <div class="col-md-6">
                <div class="card main-chart">
                    ...
                    <div class="card-body">
                        <a href="{{ route('department.delete', $admin->id ?? '') }}" class="btn btn-danger"
                            id="delete">Delete</a>
                        <a href="{{ route('department.adminEdit', $admin->id ?? '') }}" class="btn btn-primary">Edit</a>
                        <div class="text-center">
                            <div class="">
                                <img class=" border" src="                     @if (isset($admin->image) && file_exists($admin->image))
                                {{ asset($admin->image) }}@else
                            ...
                            <div>
                                পদবী: {{ $admin->designation ?? 'Unset' }}
                            </div>
                            <div>
                                মোবাইল: {{ $admin->contact ?? 'Unset' }}
                            </div>
                            <div>
                                টেলিফোন: {{ $admin->phone ?? 'Unset' }}
                            </div>
                            <div>
                                ইমেইল: {{ $admin->email ?? 'Unset' }}
                            </div>
                        </div>
                    </div>
                </div>
