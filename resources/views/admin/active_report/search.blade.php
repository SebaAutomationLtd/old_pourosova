                ...
                <div class="col-md-12">
                    <div class="card main-chart mt-4">
                        <div class="card-header panel-tabs">
                            <h5>সাপোর্ট এডমিনের একটিভ রিপোর্ট ({{ date('Y-M-d') }}) <a
                        ...
                        <div class="card-body">
                            ...
                            <div class="search">
                                <form method="get" action="{{ route('activeReport.filter') }}">
                                    @csrf
                                    <div class="form-row">
                                        ...
                                        @foreach ($users as $key => $row)
                                            <tr>
                                                ...
                                                <td>
                                                    @php
                                                        if ($ward_s == '' && $from_s == '') {
                                                            $query = 'select * from business_holds where activate_by="' . $row->id . '"';
                                                            $business = DB::select($query);
                                                            $query = 'select * from general_members where activate_by="' . $row->id . '"';
                                                            $general = DB::select($query);
                                                        } elseif ($ward_s == '' && $from_s != '') {
                                                            $query = 'select * from business_holds where activate_by="' . $row->id . '" and date(created_at) between "' . $from_s . '" and "' . $to_s . '"';
                                                            $business = DB::select($query);
                                                            $query = 'select * from general_members where activate_by="' . $row->id . '" and date(created_at) between "' . $from_s . '" and "' . $to_s . '"';
                                                            $general = DB::select($query);
                                                        } elseif ($ward_s != '' && $from_s == '') {
                                                            $query = 'select * from business_holds where activate_by="' . $row->id . '" and ward_id="' . $ward_s . '"';
                                                            $business = DB::select($query);
                                                            $query = 'select * from general_members where activate_by="' . $row->id . '" and ward_id="' . $ward_s . '"';
                                                            $general = DB::select($query);
                                                        } elseif ($ward_s !== '' && $from_s != '') {
                                                            $query = 'select * from business_holds where activate_by="' . $row->id . '" and ward_id="' . $ward_s . '" and date(created_at) between "' . $from_s . '" and "' . $to_s . '"';
                                                            $business = DB::select($query);
                                                            $query = 'select * from general_members where activate_by="' . $row->id . '" and ward_id="' . $ward_s . '" and date(created_at) between "' . $from_s . '" and "' . $to_s . '"';
                                                            $general = DB::select($query);
                                                        }
                                                ...
                                                <td>
                                                    <a data-toggle="tooltip" title="তথ্যগুলো দেখুন"
                                                       href="{{ route('activeReport.show', $row->user_id) }}"
