                       ...
                       <tbody>
                        @foreach($all as $key=>$row)
                       	 <tr>
                       	  ...
                       	  @php
                       	   $year = date('Y');
                       	   $count = DB::table('due_trade_licenses')
                       	         ->where('data_id',$row->id)
                       	         ->count();
                       	     $due_cost = $year - $row->last_license_issue_year+1;
                       	  @endphp
                       	  @if($row->last_license_issue_year == $year)
			                 <td>-</td>
			                @elseif($count > 0)
			                @for($i=$row->last_license_issue_year+1; $i<=$year; $i++)
			                <td>
			                  ...
			                  @endfor	
			                 @else
			                  <td>{{$row->last_license_issue_year}}-{{$row->last_license_issue_year+1}}</td>
			                @endif
			                ...
			                @if($count > 0)
			                 @php
			                  $show = DB::table('due_trade_licenses')
                       	         ->where('data_id',$row->id)
                       	         ->get();
			                 @endphp
			                 @foreach($show as $val)
			                ...
			                @endforeach
			                @else
			                <td>{{$due_cost * $row->trade_total;}}</td>
			                @endif
			                <td>
			                 <a style="cursor: pointer; color: white;" class="btn btn-primary trade_aday" data-id="{{$row->id}}">আদায়</a>
			                </td>
                       	 </tr>
                       	 @endforeach
                       </tbody>
 ...
 <div class="modal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="{{URL::to('/insert-trade_due')}}" method="post">
      	@csrf
...
<script>
 $(document).ready(function(){
 	$(document).on('click', '.trade_aday', function(e){
 		e.preventDefault();
 		var id = $(this).data('id');
 		$('.modal').modal('show'); 

 		 $.ajax({
                 url: "{{  url('/aday-trade_due') }}", 
